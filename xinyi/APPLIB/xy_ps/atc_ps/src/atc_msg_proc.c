...
void AtcAp_CascadeAtProc_NextAt()
{
    ...
    for(i = 0; i < pCascadeAtInfo->usLen - pCascadeAtInfo->offset; i++)
    {
        if (D_ATC_N_SEMICOLON == pCascadeAtInfo->pCasaadeAtBuff[pCascadeAtInfo->offset + i])
        {
            ...
            ATC_SendApDataReq(ATC_AP_FALSE, 0, usLength, pucData);
            AtcAp_Free(pucData);
...
static void AtcAp_DataReqProc_AtCmd(ATC_AP_MSG_DATA_REQ_STRU* pAtcDataReq)
{
    ...
    if(ATC_AP_FALSE == AtcAp_Command_Excute(g_AtcApInfo.pCurrEvent))
    {
        ...
        if(AtcAp_SendCmdEventToPs() != XY_OK)
        {
            AtcAp_SendErrorRsp();
        }
    }
...
{
    xy_printf("AtcAp_MsgProc_AtcApDataReq: cmd=%s, ucExternalFlg=%d, ulSemaId=%d",pAtcDataReq->aucMsgData, pAtcDataReq->ucExternalFlg, pAtcDataReq->ulSemaId);
...
static void AtcAp_MsgProc_AtcApDataInd(ATC_MSG_DATA_IND_STRU* pAtcDataInd)
{
    ...
    {
        xy_printf("[AtcAp_MsgProc_AtcApDataInd]: ucSeqNum=%d, event=%d, userAtCnf",pAtcDataInd->ucSeqNum, pCmdEvent->usEvent);
    }
    else
    {
        xy_printf("[AtcAp_MsgProc_AtcApDataInd]: ucSeqNum=%d, event=%d",pAtcDataInd->ucSeqNum, pCmdEvent->usEvent);
    }
