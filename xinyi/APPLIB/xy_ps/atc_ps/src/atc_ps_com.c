...
void ATC_SendApDataReq(unsigned char ucExternalFlg, unsigned long ulAppSemaId, unsigned short usDataLen, unsigned char*pucData)
{
...
void AtcAp_SendCmeeErr(unsigned short usErrCode)
{
    ...
    if(g_AtcApInfo.ucUserAtFlg == ATC_AP_FALSE)
    {
        if (D_ATC_ERRMOD_DIG_CMEE == g_softap_fac_nv->cmee_mode)
        {
            g_AtcApInfo.stAtRspInfo.usRspLen = AtcAp_StrPrintf((unsigned char *)g_AtcApInfo.stAtRspInfo.aucAtcRspBuf,
                (const unsigned char *)"\r\n+CME ERROR:%d\r\n", usErrCode);
        }
        else if (D_ATC_ERRMOD_STR_CMEE == g_softap_fac_nv->cmee_mode)
        {
...
static void AtcAp_SendAtcDateReqToNas(unsigned char *pReqMsg, unsigned short usMsgLen)
{
    ...
    PSMsg.MsgType    = LNBPS_MSG_TYPE_NORMAL;
    PSMsg.MsgPointer = (ULONG)pReqMsg;
...
int AtcAp_SndDataReqToPs(unsigned char *pCodeStream, unsigned short usCodeStreamLen)
{
    ...
    xy_printf("[AtcAp_SndDataReqToPs] ucUserAtFlg=%d, ucSeqNum=%d, event=%d", g_AtcApInfo.ucUserAtFlg, pAtcDataReq->ucSeqNum, pCmdEvent->stCgdcontParam.usEvent);
    AtcAp_SendAtcDateReqToNas(pAtcDataReq, usLen);
...
int AtcAp_SndDataReqToShm(unsigned char *pCodeStream, unsigned short usCodeStreamLen)
{
    ...
    pCmdEvent  = (UN_ATC_CMD_EVENT*)pCodeStream; 
    xy_printf("[AtcAp_SndDataReqToShm] ucUserAtFlg=%d, ucSeqNum=%d, event=%d", g_AtcApInfo.ucUserAtFlg, pInterCoreMsg->ucSeqNum, pCmdEvent->stCgdcontParam.usEvent);
...
static unsigned short AtcAp_GetCmdEventBuffSize(UN_ATC_CMD_EVENT *pCmdEvent)
{
    ...
    switch(pCmdEvent->stCgdcontParam.usEvent)
    {
        ...
        case D_ATC_EVENT_ECURC:
            return sizeof(ST_ATC_ECURC_PARAMETER);
...
static void AtcAp_OutputIpAddrAndSubMask(unsigned char ucPdpType, unsigned char* pucIpAddr, unsigned char *pucAtcRspBuf)
{
...
void AtcAp_CGCONTRDP_Print(EPS_CGCONTRDP_DYNAMIC_INFO *ptPdpDynamicInfo, unsigned char bPdpType, unsigned char *pucAtcRspBuf)
{
    ...
    {
        AtcAp_OutputIpAddrAndSubMask(bPdpType, ptPdpDynamicInfo->aucPdpAddrValue + 8, pucAtcRspBuf);
    }
    else if(D_PDP_TYPE_IPV6 == bPdpType)
    {
        AtcAp_OutputIpAddrAndSubMask(bPdpType, ptPdpDynamicInfo->aucPdpAddrValue, pucAtcRspBuf);
    }
    else
    {
...
void ATC_CmdHeaderWithSpaceProc(char** pBuff, unsigned short* pusLen, unsigned short usMaxLen)
{
    ...
    if(NULL != strstr(*pBuff, "+PSINFO:") || NULL != strstr(*pBuff, "+DBGINFO:"))
    {
        return;
    }
