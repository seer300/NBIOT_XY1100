		...
		case 0:
			...
			case D_ATC_PARAM_OK:
				...
				else
				{
					...
					ucResult = D_ATC_COMMAND_PARAMETER_ERROR;
					//PS_TRACE(0, NAS_THREAD_ID, FATAL_LOG, "[LnbAtc]!!!!!!!!!ATC_CCHO_LNB_Command line = %d!!!!!!!!!!!!!", __LINE__);
				}
				break;
			default:
				iParamNum = D_ATC_PARAM_MAX_P_CCHO;
		...
		case 0:
			...
			if (D_ATC_PARAM_OK == ucParResult)
			{
				...
				else
				{
					...
					ucResult = D_ATC_COMMAND_PARAMETER_ERROR;
					PS_TRACE(0, NAS_THREAD_ID, FATAL_LOG, "\n[LnbAtc]!!!!!!!!!ATC_CCHC_LNB_Command line = %d!!!!!!!!!!!!!\n", __LINE__);
				}
			}
			else
			{
				iParamNum = D_ATC_PARAM_MAX_P_CCHC;
				ucResult = D_ATC_COMMAND_SYNTAX_ERROR;
				PS_TRACE(0, NAS_THREAD_ID, FATAL_LOG, "[LnbAtc]!!!!!!!!!ATC_CCHC_LNB_Command line = %d!!!!!!!!!!!!!", __LINE__);
			}
			break;
	...
	for (; iParamNum < D_ATC_PARAM_MAX_P_CGLA; iParamNum++)
	{
		...
		case 0:
			...
			else
			{
				iParamNum = D_ATC_PARAM_MAX_P_CGLA;
				ucResult = D_ATC_COMMAND_PARAMETER_ERROR;
				//PS_TRACE(0, NAS_THREAD_ID, FATAL_LOG, "[LnbAtc]!!!!!!!!!ATC_CGLA_LNB_Command line = %d!!!!!!!!!!!!!", __LINE__);
			}
			break;
		case 1:
...
unsigned char ATC_CGAUTH_LNB_Command(unsigned char *pCommandBuffer, unsigned char *pEventBuffer)
{
    ...
    //auth prot
    if(D_ATC_COMMAND_OK != ATC_GetDecimalParameterByte(pCommandBuffer, &usCmdStrLen, 3, &(pCgauth->ucAuthProt), &ucParamFlg, 0, 2, 1, 0))
    {
        return D_ATC_COMMAND_PARAMETER_ERROR;
    }
...
#if VER_QUCTL260
unsigned char ATC_QPLMNS_LNB_Command(unsigned char *pCommandBuffer, unsigned char *pEventBuffer)
{
    unsigned short usCmdStrLen = 7; //+QPLMNS
    unsigned char ucCmdFunc;

    return ATC_CmdFuncInf(pCommandBuffer + usCmdStrLen,EVENT_QPLMNS, pEventBuffer, &ucCmdFunc);
}
#endif
...
unsigned char ATC_QLOCKF_LNB_Command(unsigned char *pCommandBuffer, unsigned char *pEventBuffer)
{
    ...
    case 1:
        ...
        ucParResult = ATC_CheckLongNumParameter((pCommandBuffer + usCmdStrLen), 5, &ucParLen, &usPci, &ucStopStrInf,0);

        if((D_ATC_PARAM_OK == ucParResult) && (usPci <= 503))
        {
            if(D_ATC_STOP_CHAR_CR == ucStopStrInf)
            {
               pParam->usPci = usPci;
...
unsigned char ATC_ECURC_LNB_Command(unsigned char *pCommandBuffer, unsigned char *pEventBuffer)
{
    unsigned short usCmdStrLen = 6;                                                              /* "+ECURC" length                        */
    unsigned char ucCmdFunc;                                                                    /* command form                             */
    unsigned char ucResult;                                                                     /* command result                           */
    unsigned char ucStopStrInf;                                                                 /* stop string                              */
    unsigned char ucParLen = 0;                                                                 /* parameter length                         */
    ...
    unsigned char ucParResult;                                                                  /* parameter result                         */
    unsigned char aucFunc[12]  = { 0 };
    unsigned char index;
    unsigned char i = 0;
    ST_ATC_ECURC_PARAMETER* pParam = (ST_ATC_ECURC_PARAMETER*)pEventBuffer;


    ucResult = ATC_CmdFuncInf(pCommandBuffer + usCmdStrLen,EVENT_ECURC,pEventBuffer,&ucCmdFunc);
    
    if(D_ATC_CMD_FUNC_SET == ucCmdFunc)
    {
        usCmdStrLen += 1;                                                               /* '=' set                                  */

        ucParResult = ATC_CheckStrParameter((pCommandBuffer + usCmdStrLen), 12, &ucParLen, &ucStrLen, aucFunc, &ucStopStrInf);
        ...
        {
            for(index = 0; index < D_ATC_ECURC_MAX; index++)
            {
                if(0 == strcmp((char*)aucFunc, (char*)ATC_ECURC_Table[index].aucStr))
                {
                    pParam->ucFunc = ATC_ECURC_Table[index].ucStrVal;
                    break;
                }
            }
            if(D_ATC_ECURC_MAX == index)
            {
                return D_ATC_COMMAND_PARAMETER_ERROR;
            }
        ...
        if(D_ATC_COMMAND_OK != ATC_GetDecimalParameterByte(pCommandBuffer, &usCmdStrLen, 3, &pParam->ucVal, NULL, 0, 1, 0, 1))
        {
            return D_ATC_COMMAND_PARAMETER_ERROR;
        }
...
unsigned char ATC_QNBIOTEVENT_LNB_Command(unsigned char *pCommandBuffer, unsigned char *pEventBuffer)
{
    unsigned short usCmdStrLen = 12; //+QNBIOTEVENT
    unsigned char ucResult;
    unsigned char ucCmdFunc;
    unsigned char ucEvent;
    ST_ATC_MNBIOTEVENT_PARAMETER* pParam = (ST_ATC_MNBIOTEVENT_PARAMETER*)pEventBuffer;
