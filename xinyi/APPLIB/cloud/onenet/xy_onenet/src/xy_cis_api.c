...
static int xy_get_read_value(char *value, struct onenet_read *dst)
{
	...
	{
		dst->value = xy_zalloc(strlen(value) + 1);
		cis_memcpy(dst->value, value, strlen(value));	
	...
	else if (dst->value_type == cis_data_type_string)	
	{
		...
		dst->value = xy_zalloc(dst->len + 1);
		cis_memcpy(dst->value, value, dst->len);
	} 
...
static int cis_cfg_set_len_and_value(char *cur, char *data)
{
	if (data != NULL)
	{
		*((unsigned short*)cur) = bswap_16(strlen(data));
		cur += 2;
...
//	char *Userdata2 = "AuthCode:;PSK:;";
	...
	*cur = (version << 4) + cfgcnt;
	total_len = (unsigned short*)(cur + 1);
	*total_len = 3;
	cur += 3;
	// item id 1
	*cur = 0xf1;
	sub_len = (unsigned short*)(cur + 1);
	*sub_len = bswap_16(3);
	cur += 3;
	*total_len += 3;
	// item id 2
	...
	*cur = (((LogEnabled << 3) + (LogExtOutput << 2) + LogOutputType) << 4) + LogOutputLevel;
	cur ++;
	*((unsigned short*)cur) = bswap_16(LogBufferSize);
	cur += 2;
	*sub_len += 3;
...
int cis_create(char *ip, unsigned int port, int is_bs, char* auth)
{
	...
	{
		xy_printf("[CIS] Onenet is running.....");
		return XY_OK;
	}
	...
	if(cfg_len == 0)
	{
		xy_printf("[CIS] Onenet config error");
		return XY_Err_Parameter;
	}		

	onenet_context_config = find_proper_onenet_context_config(cfg_len, 0, cfg_len);	
	if (onenet_context_config == NULL)
	{
		xy_printf("[CIS] no free onenet_context_configs.");
		if (config != NULL)
			xy_free(config);
		return  XY_ERR;
	}
	...
	if (onenet_context_config->config_hex == NULL)
	{
	   ...
	   onenet_context_config->config_hex = xy_zalloc(cfg_len);
	...
	contextRef = get_free_onet_context_ref();
	if (contextRef == NULL) {
		xy_printf("[CIS] no free onenet_context_refs.");
		if (config != NULL)
			xy_free(config);
		return XY_ERR;
	}
	...
	contextRef->onet_at_thread_id = osThreadNew((osThreadFunc_t)(onet_at_pump), contextRef, &thread_attr);
...
int cis_addobj(int objId, int insCount, char* insBitmap, int attrCount, int actCount)
{
    ...
    if (!is_onenet_task_running(0)) {
        xy_printf("[CIS] cis is not running!");
        return XY_Err_NotAllowed;
    }

	if(objId<0 || insCount<0)
	{
		xy_printf("[CIS] error:objId(%d),insCount(%d)", objId, insCount);
		goto param_error;
	}	
	
	if(insCount != strlen(insBitmap))
	{
		xy_printf("[CIS] error:insBitmap(%s),len(%d)", insBitmap, strlen(insBitmap));
		goto param_error;
	}	

	if(attrCount<0 || actCount<0)
	{
		xy_printf("[CIS] error:attrCount(%d),actCount(%d)", attrCount, actCount);
		goto param_error;
	}	
	...
	param.insBitmap = xy_zalloc(param.insCount + 1);
	cis_memcpy(param.insBitmap, insBitmap, param.insCount);
	xy_printf("[CIS] cis is addobj!");
    osMutexAcquire(g_onenet_mutex, osWaitForever);
...
int cis_cloud_setting(char *ip_addr_str, int port, char *authcode)
{
	...
	memset(g_softap_fac_nv->cloud_server_ip, 0, 16);
	...
	if(authcode!=NULL && strlen(authcode)!=0)
	{
		...
		memset(g_softap_fac_nv->cloud_server_auth, 0, 16);
		memcpy(g_softap_fac_nv->cloud_server_auth, authcode, strlen(authcode));
	}
...
ackid)
{
	...
	if(msgId == -1)
	{
		...
		xy_printf("[CIS]observe mid(%d)", notifyParam.msgId);
	...
	if(len == 0)
	{
		xy_printf("[CIS]observe at value(NULL)");
		goto param_error;
	}
	else
	{
		if (XY_OK != xy_get_notify_value(value, &notifyParam)) {
			xy_printf("[CIS]observe get value error");
			goto param_error;
		}		
	}
	...
	osMutexRelease(g_onenet_mutex);
	xy_printf("[CIS]observe errorcode(%d)", errorcode);
...
ackid)
{
    int ret = XY_ERR;
    if(ackid == 0)
        cis_set_rai_flag(PS_SOCK_RAI_NO_UL_DL_FOLLOWED);
    else
...
int cis_discover_rsp(int msg_id, int objId, int result, int length, char* valueStr)
{
	...
	if(valueStr == NULL || length != strlen(valueStr))
		goto param_error;
...
int cis_updatelife(int lifetime, bool withObjFlag)
{
    ...
    } else if (cis_lifetime < LIFETIME_LIMIT_MIN || cis_lifetime > LIFETIME_LIMIT_MAX) {
        goto param_error;
    }
