		...
		case RES_M_PACKAGE:
		{
			...
			if (contextP->isupdate_mcu == false) {
				if (data->asBuffer.length == 0)//restart a new update task
				{
					...
					{
						object_asynAckNodata(contextP, contextP->fw_request.response_ack, COAP_500_INTERNAL_SERVER_ERROR,PS_SOCK_RAI_NO_INFO);
	...
	case RES_M_UPDATE:
		if (targetP->fw_info->state == FOTA_STATE_DOWNLOADED)
		{
				...
				|| targetP->fw_info->result == FOTA_RESULT_UPDATEFAIL)//bad request
			{
				...
				object_asynAckNodata(contextP, contextP->fw_request.response_ack, COAP_204_CHANGED,PS_SOCK_RAI_NO_INFO);
