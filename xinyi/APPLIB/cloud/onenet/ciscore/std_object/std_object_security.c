	...
	st_object_t * securityObj)
{
...
#endif
	{
		...
		if((get_sys_up_stat() != POWER_ON) && (g_onenet_regInfo != NULL) && (strlen(g_onenet_regInfo->endpointname) != 0))
	    {
	        memcpy(targetP->identify, g_onenet_regInfo->endpointname, strlen(g_onenet_regInfo->endpointname));
	    }
		else{
