...
void dmSdkInit(void *DMconfig)
{
	...
	xy_get_IMEI(g_opt->szCMEI_IMEI, sizeof(g_opt->szCMEI_IMEI));
	xy_get_IMSI(g_opt->szIMSI, sizeof(g_opt->szIMSI));
    cissys_getSN(g_opt->szSN,sizeof(g_opt->szSN));
	if(strlen((const char *)g_softap_fac_nv->dm_app_key) > 0)
		cis_memcpy(g_opt->szAppKey, g_softap_fac_nv->dm_app_key, 64);
	if(strlen((const char *)g_softap_fac_nv->dm_app_pwd) > 0)
		cis_memcpy(g_opt->szPwd, g_softap_fac_nv->dm_app_pwd, 64);
	return ;
}
#if 1
...

{
    ...
    int nTotal = nBei * AES_BLOCK_SIZE;
        ...
        xy_printf("de enc_s=%p,size=%d,len=%d\n",enc_s,nTotal,nLen);
...
{	
  ...
  if(ciphertext==NULL)
  {
	...
	{
		xy_printf("pwd is null,use default pwd is:%s~\n", passwd);
	}
...
int genDmUpdateEndpointName(char **data,void *dmconfig)
{
  ...
  if(szEpname==NULL)
  {
		xy_printf("mem err u1\n");ret=-1;
		goto out;
	}
	ciphertext=( char *)cis_malloc(EP_MEM_SIZE);
  if(ciphertext==NULL)
  {
		xy_printf("mem err u2\n");ret=-1;
		goto out;
	}
	cis_memset(ciphertext,0,EP_MEM_SIZE);
    cis_memset(szEpname,0,EP_MEM_SIZE);
