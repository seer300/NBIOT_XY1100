...
#define http_info(fmt, ...)   do { softap_printf(USER_LOG, WARN_LOG, "[HTTP][%s %d] "fmt" \r\n", __FUNCTION__, __LINE__, ##__VA_ARGS__); } while (0)
#define http_err(fmt, ...)    do { softap_printf(USER_LOG, WARN_LOG, "[HTTP][%s %d] "fmt" \r\n", __FUNCTION__, __LINE__, ##__VA_ARGS__); } while (0)
#define http_debug(fmt, ...)  do { softap_printf(USER_LOG, WARN_LOG, "[HTTP][%s %d] "fmt" \r\n", __FUNCTION__, __LINE__, ##__VA_ARGS__); } while (0)
#else
