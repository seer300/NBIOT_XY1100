...
int http_ssl_conn_wrapper(httpclient_t *client, const char *host)
{
    ...
    */
    ...
    if ((ret = xy_mbedtls_net_connect(&ssl->net_ctx, host, port, MBEDTLS_NET_PROTO_TCP)) != 0) {
    ...
    mbedtls_ssl_set_bio(&ssl->ssl_ctx, &ssl->net_ctx, xy_mbedtls_net_send, xy_mbedtls_net_recv, xy_mbedtls_net_recv_timeout);
