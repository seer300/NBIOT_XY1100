...
{	
	...
	if (errorCode == HTTP_ECONN) {
		softap_printf(USER_LOG, WARN_LOG, "[HTTP] connection error occurred, ret:%d", errorCode);
		snprintf(urc_buf, 40, "\r\n+HTTPERR:%d,%d\r\n", http_context_ref->http_id, errorCode);
		
	} 
	else if (errorCode == HTTP_ECLSD) {
		softap_printf(USER_LOG, WARN_LOG, "[HTTP] connection closed by http server, ret:%d", errorCode);
		snprintf(urc_buf, 40, "\r\n+HTTPERR:%d,%d\r\n", http_context_ref->http_id, errorCode);
	}
	else {
		softap_printf(USER_LOG, WARN_LOG, "[HTTP] other error, ret:%d", errorCode);
		snprintf(urc_buf, 40, "\r\n+HTTPERR:%d,%d\r\n", http_context_ref->http_id, errorCode);
	}
	
	send_urc_to_ext(urc_buf);
	xy_free(urc_buf);
}
...
void http_recv_report(http_context_reference_t* http_context_ref, int flag)
{
	...
	if (content_len) {
		 if (http_context_ref->print_mode == HEX_CHARACTER) {
			...
			urc_buf = xy_malloc(40 + content_len);
			snprintf(urc_buf, 40 + content_len, "\r\n+HTTPNMIC:%d,%d,%d,%d\r\n%s\r\n", http_context_ref->http_id, flag, 
		                              http_context_ref->client_data->response_content_len,http_context_ref->client_data->content_block_len, http_context_ref->client_data->response_buf);
			send_urc_to_ext(urc_buf);
			xy_free(urc_buf);
		}	
	}
...
{	
	...
	if (errorCode == HTTP_SUCCESS) {
		...
		softap_printf(USER_LOG, WARN_LOG, "[HTTP] http send report, ret:%d", errorCode);
		snprintf(urc_buf, 24, "\r\n+BADREQUEST\r\n");
	}
...
HTTPC_RESULT get_free_http_context_ref(int *http_id)
{
	...
	client_data->header_buf   = xy_malloc (HTTPCLIENT_HEADER_SIZE);
	client_data->response_buf = xy_malloc (HTTPCLIENT_RESP_SIZE);
...
HTTPC_RESULT http_header_cfg(int http_id, char *header, int encode_method)
{
	...
	if ((header != NULL) && (strcmp(header, "") != 0)) {
		if (encode_method == 0) {
			len = strlen(header);
			temp_header_content = xy_malloc(len + 1);
		...
		if (http_context_ref->client->header == NULL) {
			...
			temp = xy_malloc(strlen(http_context_ref->client->header) + strlen(temp_header_content) + 1);
...
HTTPC_RESULT http_content_cfg(int http_id, char *content, int encode_method)
{
	...
	if ((content != NULL) && (strcmp(content, "") != 0)) {
		...
		if (http_context_ref->client_data->post_buf == NULL) {
			...
			temp = xy_malloc(strlen(http_context_ref->client_data->post_buf) + strlen(temp_content) + 1);
...
void http_recv_thread(void* argument)
{
	...
	{	
		...
		softap_printf(USER_LOG, WARN_LOG,"[HTTP] content_block_len:%d, response_code:%d, ret:%d", http_context_ref->client_data->content_block_len, http_context_ref->client->response_code, ret);
...
void http_send_thread(void* argument)
{
	...
	if (http_context_ref->status == HTTPSTAT_CLOSED) {
		...
		http_cert_pk_clear(http_context_ref); //由于server_cert，client_cert与client_pk是进行动态申请内存，且较大，所以连接结束以后，及时释放相关内存；
		if (ret != HTTP_SUCCESS) {
	...
	if (http_context_ref->http_recv_thread == NULL) {
		...
		task_attr.priority = XY_OS_PRIO_NORMAL1;
...
int at_http_create(char *at_buf, char **rsp_cmd)
{
	...
	*rsp_cmd = xy_malloc(24);
	...
	if (at_parse_param_3("%s,%s,%s", at_buf, &parsed_num, url, auth_user, auth_passwd) != AT_OK) {
		goto create_error;
	}
...
int at_http_cfg(char *at_buf, char **rsp_cmd)
{
	...
	if (parsed_num < 2 || parsed_num > 4 || http_id < 0 ||  http_id >= HTTP_CONTEXT_REF_NUM) {
		goto cfg_error;
	}
...
int at_http_header(char *at_buf, char **rsp_cmd)
{
	...
	if (parsed_num < 1 || parsed_num > 3 || http_id < 0 || http_id >= HTTP_CONTEXT_REF_NUM){
		goto header_error;
	}
...
int at_http_send(char *at_buf, char **rsp_cmd)
{
	...
	if (parsed_num != 3 || http_id < 0 || http_id >= HTTP_CONTEXT_REF_NUM) {
		goto send_error;
	}
...
int at_http_close(char *at_buf, char **rsp_cmd)
{
	...
	if(parsed_num != 1 || http_id < 0 || http_id >= HTTP_CONTEXT_REF_NUM){
		goto close_error;
	}
