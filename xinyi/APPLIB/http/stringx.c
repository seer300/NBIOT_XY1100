...
{
	char *target = (char*)xy_zalloc(strlen(a) + strlen(b) + 1);
	strcpy(target, a);
	strcat(target, b);
	return target;
}
...
{
	char *pstr = str, *buf = (char*)xy_zalloc(strlen(str) * 3 + 1), *pbuf = buf;
  	while (*pstr)
	{
...
{
   char *tmp = (char*)xy_zalloc(strlen(src) + 1);
   if(tmp)
       strcpy(tmp, src);
   return tmp;
}
...
{
	char *clrdst = (char*)xy_zalloc( ((strlen(b64src) - 1) / 3 ) * 4 + 4 + 50);
...
{
	char *b64dst = (char*)xy_zalloc(strlen(clrstr) + 200);
	char *tmp =NULL;
//	char b64[] = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/";
	unsigned char in[3];
	int i, len = 0;
	...
	tmp = (char *)xy_zalloc(strlen(b64dst) + 1);
	memcpy(tmp, b64dst, strlen(b64dst));
	xy_free(b64dst);
	return tmp;
}
