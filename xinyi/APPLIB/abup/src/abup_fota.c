...
static void abup_task_deinit(void)
{
	...
	{
		timer_attr.name = "abup_fota_timer";
		abup_fota_timer = osTimerNew(abup_check_version_timeout, osTimerOnce, NULL, &timer_attr);
...
static void abup_montior_start(void)
{
	...
	{
		timer_attr.name = "abup_fota_timer";
		abup_fota_timer = osTimerNew(abup_montior_timer, osTimerOnce, NULL, &timer_attr);
		osTimerStart(abup_fota_timer, 10 * 1000);
	}
	else
	{
...
static void abup_fota_task(void)
{
	...
	while (s_abup_task_data != NULL && !s_abup_task_data->is_quit)
	{
		if(abup_fota_sem == NULL)
		{
			abup_fota_sem = osSemaphoreNew(0xFFFF, 0, NULL);
...
void abup_create_fota_task(void)
{
	...
	thread_attr.name       = "abup_fota_task_thread";
	thread_attr.priority   = osPriorityNormal;
	thread_attr.stack_size = 0x800;
	abup_socket_handle = osThreadNew(abup_fota_task_thread, NULL, &thread_attr);
...
void abup_init(void)
{
	...
	{
		timer_attr.name = "abup_fota_timer";
		abup_fota_timer = osTimerNew(abup_check_update_timeout, osTimerOnce, NULL, &timer_attr);
...
void abup_task_message_send(unsigned int msg_id)
{
	...
	{
		timer_attr.name = "abup_sem_timer";
		abup_sem_timer = osTimerNew(abup_semaphore_release, osTimerOnce, NULL, &timer_attr);
