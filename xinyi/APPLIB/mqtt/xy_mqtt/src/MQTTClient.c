...
int MQTTSendPacket(MQTTClient* c, int length, Timer* timer)
{
    ...
    {
        softap_printf(USER_LOG, WARN_LOG,"[MQTT] send packet timeout  \n");
        rc = FAILURE;
    }
    return rc;
}
...
int MQTTConnectWithResults(MQTTClient* c, MQTTPacket_connectData* options, MQTTConnackData* data)
{
    ...
    if ((len = MQTTSerialize_connect(c->buf, c->buf_size, options)) <= 0)
        goto exit;
	softap_printf(USER_LOG, WARN_LOG,"[MQTT] len >0 \n");

    if ((rc = MQTTSendPacket(c, len, &connect_timer)) != SUCCESS)  // send the connect packet
        goto exit; // there was a problem
    softap_printf(USER_LOG, WARN_LOG,"[MQTT] MQTTSendPacket success rc=%d\n",rc);
