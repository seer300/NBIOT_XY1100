...
int xy_mqtt_downlink_process(MQTTClient* c,messageHandler messageHandler,int synflag)
{
    ...
    xy_printf("[MQTT] cycle packet_type = %d\n",packet_type);
    ...
    switch (packet_type)
    {
        ...
        case PUBLISH:
        {
            ...
            if (msg.qos != QOS0)
            {
                ...
                if (len <= 0)
                {
                    xy_printf("[MQTT] downlink publish Serialize pubreply fail\n");
                    rc = FAILURE;
                }
                else
                {
...
int xy_mqtt_connect(MQTTClient* c, MQTTPacket_connectData* options,int synflag)
{
    ...
    int len = 0;
      ...
      if (c->isconnected) /* don't send connect packet again if we are already connected */
          goto exit;
//    xy_printf("[MQTT] connect: now is connected\n");
...
int xy_mqtt_subscribe(MQTTClient* c, const char* topicFilter, int qos,int synflag )
{
    ...
    topic.cstring = (char *)xy_zalloc(strlen(topicFilter)+1);
...
int xy_mqtt_publish(MQTTClient* c, const char* topicName, MQTTMessage* message,int synflag)
{
    ...
    if (!c->isconnected)
    {
      xy_printf("MQTT is not connected \n");
      goto exit;
    }
    ...
    if (len <= 0)
    {
        xy_printf("len <= 0 error len=%d \n",len);
        goto exit;
    }
    if ((rc = MQTTSendPacket(c, len, &timer)) != SUCCESS) // send the subscribe packet
    {
        xy_printf("sendPacket error rc=%d \n",rc);
        goto exit;
    }
