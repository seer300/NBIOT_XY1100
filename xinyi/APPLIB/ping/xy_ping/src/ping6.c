...
void netif_event_callback(PsNetifStateChangeEvent event)
{
...
int get_netif_ip6addr(struct ping6_netif_ip6addr * s_addr)
{
	...
	ping6_tcpip_sem = osSemaphoreNew(0xFFFF, 0, NULL);
...
int _ping_ipv6(char *host_ip, u16_t data_len, u8_t timeout, unsigned int ping_num, int interval_time,int rai_val)
{
	...
	xy_regist_psnetif_callback(EVENT_PSNETIF_PING6, netif_event_callback);
...
out:
	if (sock != -1)
        close(sock);
	xy_deregist_psnetif_callback(EVENT_PSNETIF_PING6, netif_event_callback);
