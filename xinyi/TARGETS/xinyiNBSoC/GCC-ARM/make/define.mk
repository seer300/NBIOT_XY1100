...
DEFINES+= -DMODULE_VER=\"XY1100\"
DEFINES+= -DPRODUCT_VER=\"XY1100\"
...
ifeq ($(XY_SOCKET_PROXY_SUPPORT),y)
	DEFINES += -DXY_SOCKET_PROXY=1  	#协议栈BIP联网需要socket
endif
...
ifeq ($(TENCENT_SUPPORT),y)
	DEFINES += -DTENCENT_VER=1
endif
...
ifeq ($(XY_DTLS_SUPPORT),y)                      #开源DTLS库
	DEFINES += -DXY_DTLS=1
...
ifeq ($(ATMG_FORMAT),y)
	DEFINES += -DVER_QUCTL260=1
endif

ifeq ($(CTWING_VER_SUPPORT),y)
	DEFINES += -DCTWING_VER=1
	DEFINES += -DWITH_FOTA=1            #FOTA功能
	DEFINES += -DWITH_MBEDTLS=1         #DTLS功能
	DEFINES += -DWITH_LWIP=1            #开源库中走LWIP流程
	DEFINES += -DLWM2M_CLIENT_MODE=1    #LWM2M客户端模式，共有三种模式分别对应服务器、客户端还是启动服务器：LWM2M_SERVER_MODE、LWM2M_CLIENT_MODE、LWM2M_BOOTSTRAP_SERVER_MODE
	DEFINES += -DLWM2M_LITTLE_ENDIAN=1  #LWM2M字节序大小端模式：Little-endian，模式与芯片架构有关
endif
