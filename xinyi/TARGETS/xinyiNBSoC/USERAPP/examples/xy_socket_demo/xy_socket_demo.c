...
void create_socket_demo_recv_thread()
{
	...
	{
		thread_attr.name       = "at_raw_socket_demo";
		thread_attr.priority   = XY_OS_PRIO_NORMAL;
		thread_attr.stack_size = 0x800;
 		socket_demo_recv_thread = osThreadNew(socket_demo_recv_func, NULL, &thread_attr);
	}
}
...
{
	osThreadAttr_t thread_attr = {0};
	recv_lastdata_sem = osSemaphoreNew(1, 0, NULL);
	if(socket_demo_thread == NULL)
	{
		thread_attr.name       = "xy_socket_demo";
		thread_attr.priority   = XY_OS_PRIO_NORMAL;
		thread_attr.stack_size = 0x800;
		socket_demo_thread = osThreadNew((osThreadFunc_t)socket_demo_func, NULL, &thread_attr);
	}
}
