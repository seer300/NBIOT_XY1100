...
void app_tcpip_client_init()
{
	...
	if(app_skt_client_thd_msg_q == NULL)
		app_skt_client_thd_msg_q = osMessageQueueNew(10, sizeof(void *), NULL);
	if(app_skt_client_thd_h == NULL)
	{
		thread_attr.name	   = "app_skt_client_thd";
		thread_attr.priority   = XY_OS_PRIO_NORMAL1;
		thread_attr.stack_size = 0x800;
		app_skt_client_thd_h = osThreadNew((osThreadFunc_t)(client_socket_proc_thread), NULL, &thread_attr);
	}
