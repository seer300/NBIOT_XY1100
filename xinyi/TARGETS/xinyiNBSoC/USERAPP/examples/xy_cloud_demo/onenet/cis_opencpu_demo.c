...
int opencpu_send_data(int objId, int insId, int resId, int valueType, char* value, int index, int flag)
{
	...
	if(cis_demo_q == NULL)
		cis_demo_q = osMessageQueueNew(50, sizeof(void *), NULL);
...
char* value, int valueLen)
{
	...
	if(cis_demo_q == NULL)
		cis_demo_q = osMessageQueueNew(16, sizeof(void *), NULL);
	xy_assert(cis_demo_q != NULL);
...
void cis_demo_task_init()
{
    ...
    {
		thread_attr.name	   = "cis_demo_task";
		thread_attr.priority   = osPriorityNormal;
		thread_attr.stack_size = 0x800;
		g_cis_apidemo_Handle = osThreadNew ((osThreadFunc_t)(cis_demo_task),NULL,&thread_attr);
    }
    else
