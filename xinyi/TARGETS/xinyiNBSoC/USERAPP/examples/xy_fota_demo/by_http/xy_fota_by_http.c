...
#define XYDELTA_URL          "/resource/xyDelta"              //文件路径，如"/users/f"
#define HTTP_CONTENT_LENGTH  "Content-Length: "
#define SERVER_HOST          "http://fota.iinplus.com"        //HTTP服务器地址，如"http://139.224.131.190:3000"
...
extern int xy_wait_tcpip_ok(int timeout);
...
int get_fota_data(xy_http_method_e protocol, char *out_buf)
{
    int http_id = 0;//http的id
...
static void  xy_fota_http_demo()
{
    ...
    //获取差分包信息
    g_download_info.download_delta_size = get_xyDelta_size(BY_HTTP);
    ...
    {
        recv_len = get_fota_data(BY_HTTP, data_buf);
        if(recv_len)
        {
...
void xy_fota_http_demo_init()
{
    ...
    osThreadNew((osThreadFunc_t)(xy_fota_http_demo),NULL,&thread_attr);
}
#endif
