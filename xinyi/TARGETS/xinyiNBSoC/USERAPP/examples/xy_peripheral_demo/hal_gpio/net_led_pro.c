...
void net_light_led(uint32_t timer1_time, uint32_t timer2_time)
{
	...
	if(timer1 == NULL)
	{
		...
		timer1 = osTimerNew((osTimerFunc_t)(callback1), osTimerOnce, NULL, &timer_attr);
	...
	if(timer2 == NULL)
	{
		timer_attr.name = "led_tmr2";
		timer2 = osTimerNew((osTimerFunc_t)(callback2), osTimerOnce, NULL, &timer_attr);
...
void user_led_demo()
{
	...
	if((g_softap_fac_nv != NULL) && (!(g_softap_fac_nv->peri_remap_enable & 0x0100)) && (g_softap_fac_nv->led_pin <= HAL_GPIO_PIN_NUM_63))
		write_led_pro(D_ATC_CGEV_ME_PDN_DEACT);
...
int at_CGEV_led_info(char *at_buf)
{
	...
	{
		write_led_pro(D_ATC_CGEV_ME_PDN_ACT);
	}
	...
	{
		write_led_pro(D_ATC_CGEV_OOS);
	}
	else if ((at_paras = strstr(at_buf, "IS")) != NULL)
	{
		write_led_pro(D_ATC_CGEV_IS);
	}
	return AT_END;
}
...
void user_led_demo_init(void)
{
	...
	g_user_led_Handle = osThreadNew ((osThreadFunc_t)(user_led_demo),NULL,&thread_attr);
