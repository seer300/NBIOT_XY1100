...
{
	//创建信号量
	g_hal_csp_uart_poll_demo_sem = osSemaphoreNew(0xFFFF, 0, NULL);
...
void hal_csp_uart_poll_demo_task(void)
{
	...
	hal_csp_uart_poll_demo_peri_init();
...
void hal_csp_uart_poll_demo_task_init(void)
{
	...
	thread_attr.name	   = "hal_csp_uart_poll_demo_task";
	thread_attr.priority   = osPriorityNormal;
	thread_attr.stack_size = 512;
	g_hal_csp_uart_poll_demo_TskHandle = osThreadNew((osThreadFunc_t)hal_csp_uart_poll_demo_task, NULL, &thread_attr);
}
