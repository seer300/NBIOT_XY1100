...
* @ingroup	peripheral
* @brief	CSP_SPI模式Demo2
*			在这个Demo中，NSS片选端由用户通过API控制，MCU作为主设备发起通信，主从设备每次阻塞收发一个字节的数据，\n
* 			共进行8个字节的数据交换。\n
*
...
void hal_csp_spi_demo2_task_init(void)
{
	...
	thread_attr.name	   = "hal_csp_spi_demo2_task";
	thread_attr.priority   = osPriorityNormal;
	thread_attr.stack_size = 512;
	g_hal_csp_spi_demo2_TskHandle = osThreadNew((osThreadFunc_t)hal_csp_spi_demo2_task, NULL, &thread_attr);
}
