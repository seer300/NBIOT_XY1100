...
{
	//创建信号量
	g_hal_uart_IT_demo_sem = osSemaphoreNew(0xFFFF, 0, NULL);
...
void hal_uart_IT_demo_task_init(void)
{
	...
	thread_attr.name	   = "hal_uart_IT_demo_task";
	thread_attr.priority   = osPriorityNormal;
	thread_attr.stack_size = 1024;
	hal_uart_IT_demo_TskHandle = osThreadNew((osThreadFunc_t)hal_uart_IT_demo_task, NULL, &thread_attr);
}
