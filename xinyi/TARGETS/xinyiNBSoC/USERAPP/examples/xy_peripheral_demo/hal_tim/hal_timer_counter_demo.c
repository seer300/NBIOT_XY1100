...
{
	//创建信号量
	g_hal_counter_timer_sem = osSemaphoreNew(0xFFFF, 0, NULL);
...
void hal_counter_timer_work_task_init(void)
{
	...
	thread_attr.name	   = "hal_counter_timer_work_task";
	thread_attr.priority   = osPriorityNormal;
	thread_attr.stack_size = TIMER_STACK_SIZE;
	g_hal_counter_timer_TskHandle = osThreadNew((osThreadFunc_t)hal_counter_timer_work_task,NULL,&thread_attr);
}
