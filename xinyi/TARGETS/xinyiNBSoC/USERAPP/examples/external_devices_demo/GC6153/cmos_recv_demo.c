...
int32_t CMOS_SPI_Init(void)
{
	...
	g_spi_slave_sem = osSemaphoreNew(0xFFFF, 0, NULL);
...
void CMOS_RECV_TASK_INIT(void)
{
	...
	thread_attr.name	   = "coms_recv_task";
	thread_attr.priority   = osPriorityHigh;
	thread_attr.stack_size = SPI_SLAVE_STACK_SIZE;
	g_spi_slave_TskHandle = osThreadNew ((osThreadFunc_t)(CMOS_RECV_TASK),NULL,&thread_attr);
}
