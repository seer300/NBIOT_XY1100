...
static void user_rtc_timeout_cb(void *para)
{
	...
	{
		user_rtc_vol_sem = osSemaphoreNew(0xFFFF, 0, NULL);
	}
...
static void user_task_vol_demo(void *args)
{
	...
	timer_attr.name = "USER_WATCHDOG";
	g_user_dog_timer4 = osTimerNew((osTimerFunc_t)(user_dog_timeout), osTimerOnce, NULL, &timer_attr);
	while(1)
	{		
...
void user_task_volatile_demo_init()
{
	...
	thread_attr.name	   = "user_task_vol_demo";
	thread_attr.priority   = osPriorityNormal;
	thread_attr.stack_size = 0x400;
	g_user_task_vol_Handle = osThreadNew ((osThreadFunc_t)(user_task_vol_demo),NULL,&thread_attr); 
}
#endif
