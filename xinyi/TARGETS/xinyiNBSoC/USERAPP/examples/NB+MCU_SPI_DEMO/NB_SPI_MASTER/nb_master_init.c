...
void nb_master_spi_init(void)
{
	...
	nb_master_Semaphore = osSemaphoreNew(0xFFFF, 0, NULL);
...
void nb_master_task_init(void)
{
	...
	thread_attr.name	   = "nb_recv";
	thread_attr.priority   = 12;
	thread_attr.stack_size = 2048;
	nb_master_recv_TskHander = osThreadNew((osThreadFunc_t)nb_master_recv_task, NULL, &thread_attr);
	thread_attr.name	   = "nb_proc";
	thread_attr.priority   = 11;
	thread_attr.stack_size = 2048;
	nb_master_proc_TskHander = osThreadNew((osThreadFunc_t)nb_master_proc_task, NULL, &thread_attr);

	nb_master_proc_Queue = osMessageQueueNew(10, sizeof(tlv_cmd_info_t), NULL);

	nb_master_Send_Mutex = osMutexNew(NULL);

	void nb_master_test(void);
	thread_attr.name	   = "nb_master_test";
	thread_attr.priority   = 10;
	thread_attr.stack_size = 2048;
	osThreadNew((osThreadFunc_t)nb_master_test, NULL, &thread_attr);
}
