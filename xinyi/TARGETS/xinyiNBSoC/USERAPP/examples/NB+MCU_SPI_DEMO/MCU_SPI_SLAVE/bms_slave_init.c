...
void bms_slave_spi_init(void)
{
	...
	bms_slave_Semaphore = osSemaphoreNew(0xFFFF, 0, NULL);
...
void bms_slave_task_init(void)
{
	...
	thread_attr.name	   = "bms_recv";
	thread_attr.priority   = 12;
	thread_attr.stack_size = BMS_SLAVE_RECV_TASK_STACK_SIZE;
	bms_slave_recv_TskHander = osThreadNew((osThreadFunc_t)bms_slave_rcv_task, NULL, &thread_attr);
	
	thread_attr.name	   = "bms_proc";
	thread_attr.priority   = 11;
	thread_attr.stack_size = BMS_SLAVE_PROC_TASK_STACK_SIZE;
	bms_slave_proc_TskHander = osThreadNew((osThreadFunc_t)bms_slave_proc_task, NULL, &thread_attr);

	bms_slave_proc_Queue = osMessageQueueNew(10, sizeof(tlv_cmd_info_t), NULL);
