...
/*****IPC  SHM  RAM*****/
#define  RAM_ICM_BUF_START                  (0x2001BF00+SRAM_OFFSET_BASE)         //0x2001BF00,核间消息缓存
#define  RAM_ICM_BUF_LEN				    0xe00                                  //0x900 used

#define  RAM_XTEND_SBUF_START			    (RAM_ICM_BUF_START+RAM_ICM_BUF_LEN)    //0x2001CD00, 核间共享内存信息传输机制，需要考虑DSP核的cache对齐
#define  RAM_XTEND_SBUF_LEN				    0x300

//must cache line align,32BYTES
#define  PS_START_SYNC					    (RAM_XTEND_SBUF_START)  				//0x2001CD00,M3 set,indicate DSP work mode(campon/attach)
#define  SYS_UP_REASON			    	    (RAM_XTEND_SBUF_START+0x20)				//0x2001CD20,M3 set,system on reason,see SYS_START_STAT
#define  SOFT_RESET_REASON			    	(RAM_XTEND_SBUF_START+0x24)             //0x2001CD24,M3 set,soft reset reason,see RB_TYPE_E

#define  AT_UART_BAUD			    	    (RAM_XTEND_SBUF_START+0x40)             // M3 set,used for DSP core LPM
#define  PS_AT_FILTER						(RAM_XTEND_SBUF_START+0x44)				//0x2001CD44, AP write, DSP read

#define  DSP_SUSPEND			    	    (RAM_XTEND_SBUF_START+0x60)				//0x2001CD60,死机导出时挂起DSP核
...
#define  ARM_STANDBY_ENTER_XIP				(RAM_XTEND_SBUF_START+0xa0)				//0x2001CDA0, 进standby时退XIP，出standby时进standby；
...
#define  AP_ADC_FLAG				    (RAM_XTEND_SBUF_START+0x100)			//0x2001CE00,未使用ARM_DSP_MSG_STATE
#define  CP_ADC_FLAG				    (RAM_XTEND_SBUF_START+0x120)			//0x2001CE20,未使用DSP_ARM_MSG_STATE

#define  PUBLIC_KEY                         (RAM_XTEND_SBUF_START+0x140)            //0x2001CE40,sec boot read form OTP,user key

#define  FREELOCK_FLAG                      (RAM_XTEND_SBUF_START+0x160)            //0x2001CE60，M3核通知DSP核已释放锁，DSP核可进入深睡
...
#define  AP_LPM_STATE			    	    (RAM_XTEND_SBUF_START+0x200)            //0x2001CF00，未使用
#define  DSP_LPM_STATE			    	    (RAM_XTEND_SBUF_START+0x220)	        //0x2001CF20,DSP core lpm state,only linx/yus can vary
#define  DSP_LPM_MSG			    	    (RAM_XTEND_SBUF_START+0x240)	        //0x2001CF40，未使用
#define  FLASH_DYN_SYNC					    (RAM_XTEND_SBUF_START+0x260)            //0x2001CF60 解决DSP和M3同时写flash的冲突问题。bit 0,do dynamic write;bit 1,DSP proxy task have run
#define  DUMP_FLASH_SYNC					(RAM_XTEND_SBUF_START+0x280)            //DSP核死机时，通知M3核挂起，以便DSP核导出flash；该共享内存用于握手
#define  FLASH_NOTICE_FLAG                  (RAM_XTEND_SBUF_START+0x2A0)            //DSP核执行动态写时，需要挂起M3核，该标记指示M3核是否已挂起
#define  DSP_EXT_POOL                       (RAM_XTEND_SBUF_START+0x2C0)            //0x2001CFC0 M3 set,DSP external  POOL and log mem from M3 POOL
/************************END  0x2001D000***********************/
...
#define  BACKUP_MEM_CHECKSUM				0x24000FF0  //0x24000FF0
#define  BACKUP_MEM_FLAG					(BACKUP_MEM_CHECKSUM+4)    //0x24000FF4,指示checksum是否有效
#define  BACKUP_MEM_RESET_FLAG				(BACKUP_MEM_CHECKSUM+8)    //0x24000FF8,软重启原因值，参看RB_TYPE_E
...
#define  RAM_NV_VOLATILE_PS_LEN				2880
#define  RAM_NV_VOLATILE_PHY_START		    (RAM_NV_VOLATILE_PS_START+RAM_NV_VOLATILE_PS_LEN)    //0x24000b40
#define  RAM_NV_VOLATILE_PHY_LEN			608
#define  RAM_NV_VOLATILE_LPM_START		    (RAM_NV_VOLATILE_PHY_START+RAM_NV_VOLATILE_PHY_LEN)   //0x24000DA0
#define  RAM_NV_VOLATILE_LPM_LEN			64

#define  DSP_VOLATILE_ALL_LEN               (RAM_NV_VOLATILE_PS_LEN+RAM_NV_VOLATILE_PHY_LEN+RAM_NV_VOLATILE_LPM_LEN)  //0xDE0

#define  RAM_NV_VOLATILE_SOFTAP_START		(RAM_NV_VOLATILE_LPM_START+RAM_NV_VOLATILE_LPM_LEN)   //0x24000DE0,see g_softap_var_nv    
#define  RAM_NV_VOLATILE_SOFTAP_LEN			224               

#define  RAM_NV_VOLATILE_RTCLIST_START		(RAM_NV_VOLATILE_SOFTAP_START+RAM_NV_VOLATILE_SOFTAP_LEN)   //0x24000EC0 see  rtclist_var_nv_t
#define  RAM_NV_VOLATILE_RTCLIST_LEN		300             
...
#define	 NV_FLASH_DSP_VOLATILE_BASE		 		(0x27006000+FLASH_OFFSET_BASE)	   //升级或烧录时擦除          
#define  NV_FLASH_DSP_VOLATILE_LEN		 	    0x3000    

#define	 NV_FLASH_DSP_NON_VOLATILE_BASE		    (0x27009000+FLASH_OFFSET_BASE)	 	//升级或烧录时擦除
...
#define  NV_FLASH_NET_BASE                      (0x2700F000+FLASH_OFFSET_BASE)	 	//升级或烧录时擦除      
...
/*当USER_FLASH_BASE空间不够时，可以使用该宏；
*该宏会占用FOTA PACKET空间，扩展多少，相对应的FOTA PACKET空间就会减少多少；
*该额外区域供用户保存私有数据，FOTA升级时不擦除，不升级；
*FOTA PACKET空间在200K左右，请谨慎使用该宏！
*eg:占用64K
*	#define USER_EXT_FLASH_BASE 	0x271DC000
*	#define USER_EXT_FLASH_LEN_MAX	0x10000
*/
#define  USER_EXT_FLASH_BASE                         0
#define  USER_EXT_FLASH_LEN_MAX                      0

//该区域供用户保存私有数据，FOTA升级时不擦除，不升级；用户不得私自更改该区域大小范围，否则会引起FOTA等功能异常
#define  USER_FLASH_BASE                         0x271FC000
#define  USER_FLASH_LEN_MAX                      0x4000 
