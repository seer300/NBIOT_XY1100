...
typedef enum
{
	REBOOT_CAUSE_SECURITY_PMU_POWER_ON_RESET 	= POWER_ON,
	REBOOT_CAUSE_SECURITY_RESET_PIN 			= PIN_RESET,
	REBOOT_CAUSE_APPLICATION_AT 				= SOFT_RESET,
	REBOOT_CAUSE_APPLICATION_RTC 				= UTC_WAKEUP,
	REBOOT_CAUSE_SECURITY_EXTERNAL_PIN 			= EXTPIN_WAKEUP,
	REBOOT_CAUSE_SECURITY_WATCHDOG 				= WDT_RESET,
	REBOOT_CAUSE_SECURITY_RESET_UNKNOWN 		= UNKNOWN_ON,
} REBOOT_CAUSE;

typedef struct
{
	REBOOT_CAUSE reboot_num;
	char *reboot_info;
} reboot_reason_t;
 ...
 * @param LOCK_DELAY     延迟深睡锁，不受LOCK_XY_LOCAL影响，不能触发3GPP快速深睡和RAI
 * @param LOCK_EXT       外部MCU使用，首次输入AT命令时触发，会触发3GPP快速深睡和RAI，建议外部MCU用户使用芯片时，先发送AT+WORKLOCK=1申请外部锁
 * 						 释放外部锁时输入AT+WORKLOCK=0,保证外部锁配对使用
 */
typedef enum
{
	LOCK_DEFAULT,
...
/**
 * @brief 指示芯片是否从深睡唤醒的，且TAU尚未超时，PS无需执行attach，PDP激活的IP保持不变
 * 
 * @return bool，XY_OK表明PDP激活的IP地址未变
 * @note 该接口慎用！建议使用get_sys_up_stat接口
 */
