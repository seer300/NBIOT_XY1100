...
void soft_reset_by_wdt(int soft_rst_reason)
{
	...
	osCoreEnterCritical();// To prevent wdt_task change the value of 0xA0040004, then reset fail
...
uint32_t wdt_task_init()
{
	...
	{		
		...
		{
			thread_attr.name       = "wdt_task";
			thread_attr.priority   = osPriorityLow;
			thread_attr.stack_size = 0x300;
			g_wdt_TskHandle = osThreadNew ((osThreadFunc_t)(wdt_task),NULL,&thread_attr);
