...
void set_DSP_work_mode()
{
	...
	if(g_softap_fac_nv->xtal_32k==1)
	{
		g_dsp_mode_dbg |= 1 << 3;
		HWREG(PS_START_SYNC) = PS_DO_ATTACH;
	}
	else if ((get_sys_up_stat() == UTC_WAKEUP || get_sys_up_stat() == EXTPIN_WAKEUP || g_need_offtime == 1) && 
	...
	//if(s_loadup_dsp_flag == 0)
	{
		...
		//if(flash_header_valid == 1)
		{
			for(i = 0; i < (int)(flash_header.image_num); i++)
			{
				...
				{
					PRCM_Clock_Enable(PRCM_BASE, PRCM_CKG_CTL_WDT_EN | PRCM_CKG_CTL_SEMA_EN |PRCM_CKG_CTL_UART1_EN | PRCM_CKG_CTL_MCNT_EN | PRCM_CKG_CTL_CSP1_EN |PRCM_CKG_CTL_CSP2_EN | PRCM_CKG_CTL_CSP3_EN | PRCM_CKG_CTL_CSP4_EN | PRCM_CKG_CTL_DFE_EN | PRCM_CKG_CTL_ANASNS_EN | PRCM_CKG_CTL_AES_EN  | PRCM_CKG_CTL_PHYTMR_EN);
...
int shut_down_dspcore_process(void)
{
	...
	if(Get_CP_ALARM_RAM() != RTC_ALARM_INVALID)
	{
		/*
		xy_assert(0);
		lpm_ExitCritical();
