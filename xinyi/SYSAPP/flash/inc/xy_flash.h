...
extern int xy_flash_write(uint32_t addr,void *data, uint32_t size);
...
/**
 * @brief   nv擦除flash接口，如果magic num和write num都是全FF则不执行擦写动作
 * @param addr     [IN]The 待擦除地址.
 * @param size     [IN]The 待擦除长度. 
 * @return BOOL, see  @ref  xy_ret_Status_t.
 */
