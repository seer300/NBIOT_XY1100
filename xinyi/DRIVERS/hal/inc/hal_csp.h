...
typedef struct
{
    ...
    uint32_t IRDA_Idellevel;        /*!< CSP配置为IRDA时的接收数据IDEL优先级	*/

    uint32_t IRDA_Datawidth;        /*!< CSP配置为IRDA时的输出电平脉冲宽度	*/
}HAL_CSP_InitTypeDef;
...
/**
* @brief  CSP IRDA 发送数据电平脉冲宽度
*/
typedef enum {
	HAL_IRDA_DATA_WIDTH_1_6  = 1, /*!< 1.6us */
	HAL_IRDA_DATA_WIDTH_3_16 = 2  /*!< 3/16 of a bit time wide */
}HAL_IRDADataWidth_TypeDef;

/**
* @brief  CSP IRDA 接收数据idel优先级
*/
typedef enum {
	HAL_IRDA_IDEL_LEVEL_H = 1, /*!< receive high level */
	HAL_IRDA_IDEL_LEVEL_L = 2  /*!< receive low level */
}HAL_IRDAIdelLevel_TypeDef;
